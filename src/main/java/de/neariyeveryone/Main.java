        ...
        if (e.getButton() == MouseEvent.BUTTON1) {
            ...
            for (Agent agent : list) {
                ...
                if (!agent.name().equals("LOCK IN")) {
                    ...
                    action.add(new MultiAction("mouse-ctrl", command, null));
                    actions.add(action);
                    icons.add(String.format("data:image/jpeg;base64,%s", Base64.getEncoder().encodeToString(FileUtils.readFileToByteArray((new File(String.format("img/%s.webp", agent.name())))))));
                }
            }
